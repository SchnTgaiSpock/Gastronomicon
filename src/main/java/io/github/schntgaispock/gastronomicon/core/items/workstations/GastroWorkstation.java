...
public abstract class GastroWorkstation extends MenuBlock {
    ...
    public static final ItemStack CONTAINER_BORDER = new CustomItemStack(Material.PURPLE_STAINED_GLASS_PANE,
            "&dContainer");
    public static final ItemStack TOOL_BORDER = new CustomItemStack(Material.BLACK_STAINED_GLASS_PANE, "&8Tools");
    public static final ItemStack OUTPUT_BORDER = new CustomItemStack(Material.ORANGE_STAINED_GLASS_PANE, "&6Output");
    public static final ItemStack CRAFT_BUTTON = new CustomItemStack(Material.ORANGE_STAINED_GLASS_PANE,
            "&aClick to craft");
    ...
    protected void onNewInstance(BlockMenu menu, Block b) {
        ...
        menu.addMenuClickHandler(CRAFT_BUTTON_SLOT, (player, slot, item, action) -> {
            ...
            final ItemStack[] ingredients = Arrays.stream(getInputSlots()).mapToObj(s -> menu.getItemInSlot(s).asOne())
                    .toArray(ItemStack[]::new);
            final List<ItemStack> containers = Arrays.stream(getContainerSlots()).mapToObj(s -> menu.getItemInSlot(s).asOne())
                    .toList();
            final List<ItemStack> tools = Arrays.stream(getContainerSlots()).mapToObj(s -> menu.getItemInSlot(s).asOne())
                    .toList();
            ...
            final SlimefunItem sfItem = SlimefunItem.getByItem(recipe.getOutputs()[0]);
            final ItemStack output;
            if (sfItem == null) {
                output = recipe.getOutputs()[0];
            } else {
                final double proficiency = Gastronomicon.getInstance().getPlayerData()
                        .getInt(player.getUniqueId() + ".proficiencies." + sfItem.getId());
            ...
            menu.addItem(freeSlot, output);
            ItemUtil.returnItems(player, Arrays.copyOfRange(recipe.getOutputs(), 2, recipe.getOutputs().length - 1));
